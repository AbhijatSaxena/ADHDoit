...
export const useCommentStore = create<CommentState>((set, get) => ({
  ...
  load: async (todoId: string) => {
    ...
    const comments = await fetchComments(uid(), todoId) as TodoComment[]
    ...
    if (stored !== actual) {
      ...
      await setCommentCount(uid(), todoId, actual)
    }
  },
  ...
  add: async (text: string) => {
    ...
    const id = await addComment(uid(), todoId, payload)
    set(state => ({ comments: [...state.comments, { id, ...payload }] }))
    await bumpCommentCount(uid(), todoId, 1)
  ...
  remove: async (commentId: string) => {
    const todoId = get().todoId
    if (!todoId) return
    await deleteComment(uid(), todoId, commentId)
